...
class HOTPTest extends PHPUnit_Framework_TestCase
{
    ...
    public function testGenerate()
    {
        ...
        $otps = array(328482, 812658,  73348, 887919, 320986,
                      435986, 964213, 267638, 985814,   3773,
                      341298, 818485, 657398,  91297, 820368,
                      525990, 304831, 129574, 832989, 409881);

        $hotp = new HOTP($key);
        for ($i = 0, $n = count($otps); $i < $n; $i += 1)
            $this->assertEquals($otps[$i], $hotp->generate($i));

        $key = hex2bin("00112233445566778899");
        $otps = array(184487, 513982, 684146, 973749,  71087,
